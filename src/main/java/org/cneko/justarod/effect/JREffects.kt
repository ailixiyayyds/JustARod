...
class JREffects {
    companion object{
        val ORGASM_EFFECT: StatusEffect = Registry.register(
            Registries.STATUS_EFFECT,
            Identifier(MODID, "orgasm"),
            OrgasmEffect()
        )
        val LUBRICATING_EFFECT: StatusEffect = Registry.register(
            Registries.STATUS_EFFECT,
            Identifier(MODID, "lubricating"),
            LubricatingEffect()
        )
        val ESTRUS_EFFECT: StatusEffect = Registry.register(
            Registries.STATUS_EFFECT,
            Identifier(MODID, "estrus"),
            EstrusEffect()
        )
        val STRONG_EFFECT: StatusEffect = Registry.register(
            Registries.STATUS_EFFECT,
            Identifier(MODID, "strong"),
            StrongEffect()
        )
        val FAINT_EFFECT: StatusEffect = Registry.register(
            Registries.STATUS_EFFECT,
            Identifier(MODID, "faint"),
            FaintEffect()
        )
        val PREGNANT_EFFECT: StatusEffect = Registry.register(
            Registries.STATUS_EFFECT,
            Identifier(MODID, "pregnant"),
            PregnantEffect()
        )
        val AIDS_EFFECT: StatusEffect = Registry.register(
            Registries.STATUS_EFFECT,
